...
class ClassPeakDivision:
    def __init__(self, parameterList, outputFilesPath):
        assert len(parameterList) == 5, "ClassPeakDivision参数个数不对!"
        self.RemoveFPId = parameterList[0]
        self.RemoveFPPlotResult = parameterList[1]
        self.PeakDivNoiseThreshold = parameterList[2]
        self.PeakDivRelIntensity = parameterList[3]
        self.PeakDivMinimalPeakWidth = parameterList[4]
        # 用户选择的文件的生成位置
    ...
    def PeakDivPrepareParams(self, dataProcessing, ContinueList):
        ...
        """
        assert len(dataProcessing) == len(ContinueList), "Error in PeakDivPrepareParams!"
        ...
        for i in range(len(ContinueList)):
            ...
            if len(ContinueItem) == 1:  # 说明该图为空图
                ...
                redList, areas = self.PeakDivSplit(rawData[i], smoothData[i], orderValueList[i])
            parameters = [redList, areas, orderStrList[i], maxValueList[i]]
